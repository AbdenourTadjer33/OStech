...
import { capitalize, parseDatetime } from "@/Logic/helper";

const EditForm = ({ setUpdateModal, updateModal }) => {
	const { coupon } = updateModal;

	const { data, setData, errors, processing, post } = useForm({
		type: coupon.type || "",
		value: coupon.value || "",
		startAt: parseDatetime(coupon.start_at) || "",
		expireAt: parseDatetime(coupon.expire_at) || "",
	...
	const submitHandler = (e) => {
		...
		post(route("admin.coupon.update", { id: coupon.id }), {
			onSuccess: () => setUpdateModal({ status: false }),
		});
	};
	return (
		<form className="space-y-5" onSubmit={submitHandler}>
			...
			<div className="flex gap-4">
				<Button
					onClick={() => setUpdateModal({ status: false })}
				...
				<Button disabled={processing} className="w-full justify-center">
					Editer
				</Button>
			</div>
		</form>
