...
import { EditBrandFormContext } from "./EditForm";

const LogoEdit = () => {
    const {setData, data} = useContext(EditBrandFormContext);
    ...
    const deleteImg = async () => {
        ...
        try {
            const response = await axios.post(route("api.destroy.temp"), {
                path: data.image,
            });
    ...
    const editImg = async (cropInfo) => {
        setIsLoading(true);
        try {
            const response = await axios.post(route("api.edit.temp"), {
                path: data.image,
                cropInfo,
            });
            if (response.status === 200) {
